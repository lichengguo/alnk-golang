...
接口做的事情就像是定义一个协议（规则），只要一台机器有洗衣服和甩干的功能，就称它为洗衣机
不关心属性（数据），只关心行为（方法）
...
如果后续再加上猪、青蛙等动物的话，代码还会一直重复下去

那能不能把它们当成“能叫的动物”来处理呢？
Go语言中为了解决类似上面的问题，就设计了接口这个概念
接口区别于我们之前所有的具体类型，接口是一种抽象的类型
当看到一个接口类型的值时，你不知道它是什么，唯一知道的是通过它的方法能做什么
...
接口名：
使用type将接口定义为自定义的类型名。Go语言的接口在命名时，一般会在单词后面添加er
...
[实现接口的条件]
一个对象只要 全部实现 了接口中的方法，那么就实现了这个接口
