...
ConcreteStrategy（具体策略类）：它实现了在抽象策略类中声明的算法，
...
*/
package _2_policy_mode

import "fmt"

//==================================================================================================
//武器策略（抽象的策略）
type WwaponStrategy interface {
	UseWeapon() //使用武器
}
...
//环境类
type Hero struct {
	strategy WwaponStrategy //用手一个抽象的策略
}

//设置一个策略
func (h *Hero) SetWeaponStrategy(s WwaponStrategy) {
	h.strategy = s
}
...
func (sb *StrategyB) GetPrice(price float64) float64 {
	fmt.Println("执行策略B，所有商品满200减100")
